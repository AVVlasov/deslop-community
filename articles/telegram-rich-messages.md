...
11 июня Telegram выкатил [Bot API 10.1](https://core.telegram.org/bots/api-changelog), и весь этот цирк можно закрывать. Дальше по порядку: что выкатили, почему это ломает привычную архитектуру ИИ-ботов, три бота, которые собираются за вечер, и грабли, на которые я уже наступил за тебя.

> **TL;DR, что заберёшь из статьи:**
>
> - 💬 **Ответ бота стал документом**: таблицы, формулы, карты, сворачиваемые секции. Нативно, без webview. 32 768 символов вместо 4096.
...
> - ✅ **Грабли собраны** (часть 5). Живое демо: @RichTextDemoBot.
...
Годами всё было так. Нужен твоему ИИ-продукту настоящий интерфейс, с таблицами, структурой, стримингом? Строишь Mini App. Или webview. Или отдаёшь PDF. Само сообщение было слишком тупым контейнером для результата работы модели, поэтому ты писал целый веб-апп ради того, чтобы красиво показать одну табличку.
...
flowchart TD
    ...
    Q -->|"после 10.1"| R["RichBlock-и прямо в сообщении:<br/>таблицы, формулы, details, карты"]
    W --> U["Пользователь"]
    R --> U
...
| PDF-отчёт | формулы и таблицы «на печать» | LaTeX и таблицы в самом ответе |
...
Думалка только в стриме. Нативный `RichBlockThinking` в обычный `sendRichMessage` не проходит: API возвращает `RICH_MESSAGE_BLOCK_UNSUPPORTED`. Блок живёт только внутри стриминга через `sendRichMessageDraft`. Хочешь показать рассуждения в статичном сообщении — конвертируй в курсив или прячь в details.
